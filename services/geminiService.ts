...
const fileToPart = (file: ReferenceFile) => ({
...
export const fetchTrendingKeywords = async (field: string): Promise<string[]> => {
  ...
  return (response.text || "").split(',').map(s => s.trim().replace(/^\d+\.\s*/, '')).filter(Boolean).slice(0, 3);
};
...
export const fetchBestArticleUrl = async (keyword: string): Promise<{url: string, title: string}> => {
  ...
  if (chunk?.web) return { url: chunk.web.uri, title: chunk.web.title || keyword };
...
${READABILITY_GUIDELINES}`;
  ...
  if (params.referenceFiles) params.referenceFiles.forEach(f => parts.push(fileToPart(f)));
...
export const generateLecturePlan = async (params: PlanningParams): Promise<string> => {
  ...
  const interviewInfo = params.interviews?.map((it, i) => 
...
export const generateEventPost = async (params: EventParams): Promise<string> => {
  ...
  const interviewText = params.interviews?.map(i =>
...
export const generateInformativePost = async (params: InformativeParams): Promise<string> => {
  ...
  const interviewSection = params.interviews
    ...
    .map(i => `- ${i.name} (${i.affiliation || '관계자'}): ${i.dialogue ? `"${i.dialogue}"` : '(문맥에 맞는 전문적인 코멘트를 생성하여 삽입)'}`)
    .join('\n') || '없음';
...
export const generateCorporatePost = async (params: CorporateParams): Promise<string> => {
  ...
  const interviewSection = params.interviews
    ?.filter(i => i.name.trim())
    .map(i => `- ${i.name} (${i.affiliation || '관계자'}): ${i.dialogue ? `"${i.dialogue}"` : '(문맥에 맞는 신뢰감 있는 코멘트를 생성하여 삽입)'}`)
    .join('\n') || '없음';
...
export const generateInterviewPost = async (params: InterviewParams): Promise<string> => {
  ...
  const additionalInterviews = params.interviews
    ?.filter(i => i.name.trim())
    .map(i => `- ${i.name} (${i.affiliation || '소속미정'}): ${i.dialogue ? `"${i.dialogue}"` : '(문맥에 맞는 자연스러운 대사를 생성하여 삽입)'}`)
    .join('\n') || '없음';
...
4. **인터뷰 인용(필수)**:
   ...
   - **관계자 코멘트 필수**: 기사 마지막 단락에는 반드시 관계자(또는 주최 측)의 코멘트를 넣어 마무리하십시오.
     (예시: 재노스쿨 관계자는 "단순한 이론 전달을 넘어..."라고 설명했다. 이어 "실질적인 대응 전략과..."라고 덧붙였다.)
...
- 참고URL: ${params.refUrls.filter(u => u.trim()).join(', ')}
...
${READABILITY_GUIDELINES}`;
  ...
  return { text: res.text || "", groundingSources: res.candidates?.[0]?.groundingMetadata?.groundingChunks };
};
